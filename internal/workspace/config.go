...
func WriteTemplate(dir string, known *Config) error {
	...
	content := strings.Join([]string{
		...
		"# ---------------------------------------------------------------------------",
		"# go_private",
		"#   Default false. Set to true when your Go modules are private. `devkit ngs`",
		"#   then adds the organisation part of module_prefix to GOPRIVATE, so `go get`",
		"#   fetches them with your git credentials instead of the public Go proxy.",
		"#",
		"#   默认 false。当你的 Go module 是私有仓库时设为 true。`devkit ngs` 会把 module_prefix 的",
		...
		"#     NacosAddr  the Nacos on your own machine, written to conf/local.yaml;",
		...
		"#",
		"#     Port       写入 conf/*.yaml 的监听端口；默认 \"8888\"",
		"#     NacosAddr  你本机上的 Nacos 地址，写入 conf/local.yaml；默认 127.0.0.1:8848",
		"#     CI         生成哪种 CI 配置：github、gitlab、both、none；",
		"#                默认根据 module_prefix / idl_repo 的域名判断",
		"# ---------------------------------------------------------------------------",
		"# vars:",
		"#   Port: \"8888\"",
