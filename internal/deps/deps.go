// Package deps checks the tools a service needs (git, Go, kitex, thriftgo,
// and hz for API services) and installs the missing ones, so that `devkit ngs` works on a
...
type Want struct {
	...
	// HzVersion is set when the Hertz generator is needed too: API services
	// (`devkit nas`). Empty: hz is neither checked nor installed.
	HzVersion string
}

type generator struct{ name, flag, pkg, version string }

// generators lists the code generators want asks for, with the team's versions.
func generators(want Want) []generator {
	out := []generator{
	...
	if want.HzVersion != "" {
		out = append(out, generator{"hz", "--version", "github.com/cloudwego/hertz/cmd/hz", want.HzVersion})
	}
	return out
}
...
func Check(ctx context.Context, want Want) []Status {
	...
	out := []Status{gitStatus(ctx), goStatus(ctx)}
	for _, tool := range generators(want) {
		st := toolStatus(ctx, tool.name, tool.flag)
		if !st.Missing && !SameVersion(st.Version, tool.version) {
			st.Wanted = tool.version
		}
		out = append(out, st)
	}
	return out
